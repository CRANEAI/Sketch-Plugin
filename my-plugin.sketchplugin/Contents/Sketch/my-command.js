...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  onSetUp(context); // Set up sign up window
  ...
  userInput.addButtonWithTitle('Cancel'); // activate sign in popup

  var responseCode = userInput.runModal(); // put results of sign in into an object
  ...
  fetch('http://159.89.34.47:5002/api/v1/user/login', {
    ...
    return sendData(json.data.token);
  }).catch(function (err) {
...
function sendData(token) {
  ...
  var Document = sketch.fromNative(context.document); // put the documnent and token in the data object for the post request
